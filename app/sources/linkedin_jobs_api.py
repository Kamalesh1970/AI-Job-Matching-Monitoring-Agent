"""
LinkedIn Jobs API (RapidAPI) job source fetcher client.
...
class LinkedInJobsApiSource(BaseJobSource):
    """
    Client for fetching job postings via LinkedIn Jobs API on RapidAPI.
    Requires X-RapidAPI-Key and X-RapidAPI-Host headers.
    """

    DEFAULT_HOST = "linkedin-jobs-api.p.rapidapi.com"
    ...
    def is_enabled(self, config: Optional[Any] = None) -> bool:
        if config is not None:
            if hasattr(config, "source_linkedin_jobs_api_enabled") and not config.source_linkedin_jobs_api_enabled:
                return False
            key = getattr(config, "linkedin_jobs_api_key", "") or self.api_key
    ...
    def fetch_jobs_raw(
        ...
        if not self.api_key:
            logger.warning("LinkedIn Jobs RapidAPI key missing; skipping request.")
            return []
        ...
        try:
            ...
            logger.error("Error fetching from LinkedIn Jobs RapidAPI: %s", sanitized)
            return []
    ...
    def fetch_source_jobs(
        ...
        **kwargs: Any,
        ...
        if config is not None:
            effective_key = getattr(config, "linkedin_jobs_api_key", "") or self.api_key
            effective_host = getattr(config, "linkedin_jobs_rapidapi_host", "") or self.rapidapi_host
        ...
        if not effective_key:
            return SourceResult(
                ...
                jobs=[],
                total_fetched=0,
                error_message="LinkedIn Jobs RapidAPI key not configured.",
